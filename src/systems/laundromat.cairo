...
mod laundromat {
    ...
    // use rollyourown::utils::payout_items::add_items_payout;  // commented out for cartel-game-stage1
    ...
    impl LaundromatImpl of super::ILaundromat<ContractState> {
        fn register_score(
            ...
            //         bushido_store.progress(player_id.into(), Tasks::OG, 1, starknet::get_block_timestamp());
            //     }
            ...
            //     if weapon.suffix > 0 && weapon.suffix == clothe.suffix && weapon.suffix == foot.suffix && weapon.suffix == vehicle.suffix {
            //         bushido_store.progress(player_id.into(), Tasks::GEAR_FROM, 1, starknet::get_block_timestamp());
            //     }
            //     let items_disp = IDopewarsItemsLibraryDispatcher { class_hash: world.dns_class_hash(@"DopewarsItems_v0").unwrap() };
            ...
            //     if weapon_tier == clothe_tier && weapon_tier == foot_tier && weapon_tier == vehicle_tier {
            //         let task = match weapon_tier { 0 => panic!("invalid tier"), 1 => Tasks::FULL_LATE, 2 => Tasks::FULL_MID, 3 => Tasks::FULL_EARLY, _ => panic!("invalid tier") };
            //         bushido_store.progress(player_id.into(), task, 1, starknet::get_block_timestamp());
            //     }
            // }
        }
        ...
        fn claim(self: @ContractState, player_id: ContractAddress, game_ids: Span<u32>) {
            let world = self.world(@ns());
            // let mut dope_world = self.world(@"dope");  // commented out for cartel-game-stage1
